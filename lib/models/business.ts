...
export class BusinessModel {
  static async create(businessData: Omit<Business, "_id" | "createdAt" | "updatedAt">): Promise<ObjectId> {
  ...
  static async findById(id: string | ObjectId): Promise<Business | null> {
    const db = await connectToDatabase()
    return await db.collection("businesses").findOne({ _id: new ObjectId(id) })
  }

  static async findBySlug(slug: string): Promise<Business | null> {
    const db = await connectToDatabase()
    return await db.collection("businesses").findOne({ slug })
  }
    ...
    if (filters.location) {
      query.location = {
        $near: {
          ...
          $maxDistance: filters.location.radius * 1000, // km to meters
        },
      }
    }
